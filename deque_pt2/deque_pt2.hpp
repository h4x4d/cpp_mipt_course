...
void Deque<T, Allocator>::push_front(T&& value) {
  ...
  allocator_traits::construct(allocator_, &operator[](-1), std::move(value));
...
void Deque<T, Allocator>::push_front(const T& value) {
  ...
  allocator_traits::construct(allocator_, &operator[](-1), value);
  start_shift_ = absolute_index;
  size_++;
}
...
void Deque<T, Allocator>::push_back(T&& value) {
  ...
  allocator_traits::construct(allocator_, &operator[](size()),
                              std::move(value));
  size_++;
}
...
void Deque<T, Allocator>::push_back(const T& value) {
  ...
  allocator_traits::construct(allocator_, &operator[](size()), value);
  size_++;
}
...
void Deque<T, Allocator>::reallocate_buckets() {
  ...
  std::vector<bucket> created_buckets(big_buckets_amount, nullptr);
  size_t now = 0;
  ...
  buckets_ = created_buckets;
}

template <typename T, typename Allocator>
void Deque<T, Allocator>::init_any_bucket(size_t bucket_index,
...
Deque<T, Allocator>& Deque<T, Allocator>::operator=(Deque&& other) {
  Deque<T, Allocator> tmp = std::move(other);
  swap(tmp);
  return *this;
}
...
Deque<T, Allocator>& Deque<T, Allocator>::operator=(const Deque& other) {
  ...
  std::vector<bucket> replace_buckets(other.buckets_amount_);
